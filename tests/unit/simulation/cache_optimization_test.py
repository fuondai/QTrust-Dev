...
"""
Cache Optimization Test Module

This module tests the performance impact of caching in the blockchain environment.
It compares execution time and throughput with and without caching enabled.
...
# Create chart path utility function
def get_chart_path(filename, category='test'):
    """Get the path for saving a chart."""
    chart_dir = os.path.join(project_root, 'charts', category)
...
def plot_performance_comparison(stats_with_cache, stats_without_cache):
    ...
    # Save chart
    chart_path = get_chart_path("cache_performance_comparison.png", "test")
